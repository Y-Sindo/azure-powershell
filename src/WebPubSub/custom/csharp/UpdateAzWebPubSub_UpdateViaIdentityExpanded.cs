...
namespace Microsoft.Azure.PowerShell.Cmdlets.WebPubSub.Cmdlets
{
    ...
    public partial class UpdateAzWebPubSub_UpdateViaIdentityExpanded
    {
        partial void overrideOnDefault(HttpResponseMessage responseMessage, Task<IErrorResponse> errorResponseTask, ref Task<bool> returnNow)
        {
            ...
            WriteError(new ErrorRecord(new RestException<IErrorResponse>(responseMessage, errorResponseTask.Result), errorResponse.Code, ErrorCategory.InvalidOperation, new { ParametersBody }));
